...
std::vector < std::string > classes = { "person", "bicycle", "car", "motorcycle", "airplane", "bus", "train",
...
//std::string yolov5trt() {
    cudaSetDevice(DEVICE);
	std::string result="";
    //std::string wts_name = "D:\\yolov5s555.wts";
    std::string engine_name = "D:\\yolov5s555.engine";
    bool is_p6 = false;
    float gd = 0.33f, gw = 0.50f;
    std::string img_dir  = "D:\\yolov5-4.0\\data\\images\\";;
    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        for (int b = 0; b < fcount; b++) {
            ...
            for (size_t j = 0; j < res.size(); j++) {
				...
				std::cout << classes[((int)res[j].class_id)] << std::endl;
				//result = std::to_string((int)res[j].class_id);
			}
			std::string path = file_names[f - fcount + 1 + b];
			int first = path.find_last_of(".jpg");
           cv::imwrite(std::to_string(first) +".jpg", img);
        }
        fcount = 0;
    }
